describe("JoeScenario", () => {
  let root, accounts;
  let joe;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    joe = await deploy("JoeScenario", [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await joe.methods
            .generateCheckpoints(amt, offset)
            .send({ from: root, gas: 200000000 });
          remaining -= amt;
          offset += amt;
        }

        let result = await joe.methods.getPriorVotes(root, 1).send();
