...
describe("admin / _setPendingAdmin / _acceptAdmin", () => {
  ...
  describe("_setPendingAdmin()", () => {
    it("should only be callable by admin", async () => {
      expect(
        await send(jToken, "_setPendingAdmin", [accounts[0]], {
          from: accounts[0],
        })
    ...
    it("should properly set pending admin", async () => {
      expect(await send(jToken, "_setPendingAdmin", [accounts[0]])).toSucceed();

      // Check admin stays the same
      expect(await call(jToken, "admin")).toEqual(root);
      expect(await call(jToken, "pendingAdmin")).toEqual(accounts[0]);
    });

    it("should properly set pending admin twice", async () => {
      expect(await send(jToken, "_setPendingAdmin", [accounts[0]])).toSucceed();
      expect(await send(jToken, "_setPendingAdmin", [accounts[1]])).toSucceed();

      // Check admin stays the same
      expect(await call(jToken, "admin")).toEqual(root);
      expect(await call(jToken, "pendingAdmin")).toEqual(accounts[1]);
    });

    it("should emit event", async () => {
      const result = await send(jToken, "_setPendingAdmin", [accounts[0]]);
  ...
  describe("_acceptAdmin()", () => {
    it("should fail when pending admin is zero", async () => {
      expect(await send(jToken, "_acceptAdmin")).toHaveTokenFailure(
        "UNAUTHORIZED",
    ...
    it("should fail when called by another account (e.g. root)", async () => {
      ...
      // Check admin stays the same
      expect(await call(jToken, "admin")).toEqual(root);
      expect(await call(jToken, "pendingAdmin")[accounts[0]]).toEqual();
    });

    it("should succeed and set admin and clear pending admin", async () => {
      expect(await send(jToken, "_setPendingAdmin", [accounts[0]])).toSucceed();
      expect(
        await send(jToken, "_acceptAdmin", [], { from: accounts[0] })
      ).toSucceed();

      // Check admin stays the same
      expect(await call(jToken, "admin")).toEqual(accounts[0]);
      expect(await call(jToken, "pendingAdmin")).toBeAddressZero();
    });

    it("should emit log on success", async () => {
      expect(await send(jToken, "_setPendingAdmin", [accounts[0]])).toSucceed();
      const result = await send(jToken, "_acceptAdmin", [], {
        from: accounts[0],
      });
