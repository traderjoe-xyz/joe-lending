...
async function fillMasterChef(jToken, amount) {
  const masterChefAddress = await call(jToken, "masterChef", []);
  const masterChef = await saddle.getContractAt(
    "MasterChef",
    masterChefAddress
  );
  await send(jToken.underlying, "transfer", [masterChefAddress, amount]);
  await send(masterChef, "harnessSetUserAmount", [0, jToken._address, amount]);
}

describe("JToken", function () {
  let jToken, root, minter, borrower, benefactor, accounts;
  beforeEach(async () => {
  ...
  describe("borrowFresh", () => {
    ...
    it("proceeds if joetroller tells it to", async () => {
      await fillMasterChef(jToken, borrowAmount);
      expect(await borrowFresh(jToken, borrower, borrowAmount)).toSucceed();
    });
    ...
    it("transfers the underlying cash, tokens, and emits Transfer, Borrow events", async () => {
      ...
      expect(
        await balanceOf(jToken.underlying, masterChefAddress)
      ).toEqualNumber(beforeProtocolCash.minus(borrowAmount));
  ...
  describe("borrow", () => {
    ...
    it("gets no joe reward when borrowing", async () => {
      ...
      expect(await borrow(jToken, borrower, borrowAmount)).toSucceed();
      expect(await balanceOf(joe, borrower)).toEqualNumber(avaxUnsigned(0));
    });
  });
    ...
    [true, false].forEach((benefactorIsPayer) => {
      ...
      describe(label, () => {
        ...
        it("reverts if doTransferIn fails", async () => {
          ...
          await expect(
            repayBorrowFresh(jToken, payer, borrower, repayAmount)
          ).rejects.toRevert("revert unexpected EIP-20 transfer in return");
        });
  ...
  describe("repayBorrow", () => {
    ...
    it("gets no joe reward when repaying", async () => {
      const joeAddress = await call(jToken, "joe", []);
      const masterChefAddress = await call(jToken, "masterChef", []);
