...
`.trim();
    ...
    expressions.forEach((expression) => {
      ...
      if (expression.subExpressions.length > 0) {
        printer.printMarkdown(
          `For more information, run: \`Help ${path} ${expression.name}\``
        );
      }
    });
  } else {
    ...
    let expression = expressions.find(
      (expression) =>
        expression.name.toLowerCase() === expressionName.toLowerCase()
    );
    ...
    if (expression) {
      ...
      printHelp(
        ...
        path.concat(expression.name)
      );
    } else {
      let matchingExpressions = expressions.filter((expression) =>
        expression.name.toLowerCase().startsWith(expressionName.toLowerCase())
      );

      if (matchingExpressions.length === 0) {
        printer.printLine(
          `\nError: cannot find help docs for ${path
            .concat(<string>first)
            .join(" ")}`
        );
      } else {
