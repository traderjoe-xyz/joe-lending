...
export async function getInterestRateModelAddress(
  world: World,
...
export async function getBorrowRate(
  ...
  return new NumberV(
    await interestRateModel.methods
      .getBorrowRate(cash.encode(), borrows.encode(), reserves.encode())
      .call(),
    1.0e18 / 31536000
  );
}
...
export function interestRateModelFetchers() {
  return [
    new Fetcher<{ interestRateModel: InterestRateModel }, AddressV>(
      ...
      (world, { interestRateModel }) =>
        getInterestRateModelAddress(world, interestRateModel),
      { namePos: 1 }
    ),
    ...
    >(
      ...
      (world, { interestRateModel, cash, borrows, reserves }) =>
        getBorrowRate(world, interestRateModel, cash, borrows, reserves),
      { namePos: 1 }
    ),
  ];
}

export async function getInterestRateModelValue(
  world: World,
  event: Event
): Promise<Value> {
