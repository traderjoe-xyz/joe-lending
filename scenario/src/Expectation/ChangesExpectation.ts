...
function asNumberV(v: Value): NumberV {
  if (v instanceof NumberV) {
    ...
    throw new Error(
      `Expected NumberV for ChangesExpectation, got ${v.toString()}`
    );
  }
}
...
export class ChangesExpectation implements Expectation {
  ...
  async checker(world: World, initialCheck: boolean = false): Promise<void> {
    ...
    if (this.tolerance.val != 0) {
      if (
        Math.abs(
          Number(trueDelta.sub(this.delta).div(this.originalValue).val)
        ) > Number(this.tolerance.val)
      ) {
        fail(
          world,
          `Expected ${trueDelta.toString()} to approximately equal ${this.delta.toString()} within ${this.tolerance.toString()}`
        );
      }
    } else if (!currentValue.compareTo(world, this.expected)) {
      fail(
        world,
        `${this.toString()} instead had value \`${currentValue.toString()}\` (true delta: ${trueDelta.toString()})`
      );
    }
  }
