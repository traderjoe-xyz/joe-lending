...
import { joeCommands, processJoeEvent } from "./Event/JoeEvent";
...
import { JoeLens } from "./Contract/JoeLens";
...
export async function processEvents(
  ...
  return events.reduce(
    async (pWorld: Promise<World>, event: Event): Promise<World> => {
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] =
  [
    ...
    new Command<{ event: EventV }>(
      ...
      * "Joe ...event" - Runs given joe event
      * E.g. "Joe Deploy"
    `,
      "Joe",
      [new Arg("event", getEventV, { variadic: true })],
      (world, from, { event }) => {
        return processJoeEvent(world, event.val, from);
      },
      { subExpressions: joeCommands() }
    ),

    buildContractEvent<Counter>("Counter", false),
    buildContractEvent<JoeLens>("JoeLens", false),
