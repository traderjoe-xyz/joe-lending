  ...
  const fetchers = [
    new Fetcher<{ name: StringV; rate: NumberV }, InterestRateModelData>(
      `
        ...
        * "Fixed name:<String> rate:<Number>" - Fixed interest **per-second** rate
      ...
      async (world, { name, rate }) => ({
        invokation: await FixedInterestRateModel.deploy<InterestRateModel>(
          world,
          from,
          [rate.encode()]
        ),
        ...
        description: `Fixed rate ${rate.show()} per second`,
      })
    ),
    ...
    >(
      ...
      async (
        world,
        { name, baseRate, multiplier, jump, kink, roof, admin }
      ) => ({
        invokation: await JumpRateModel.deploy<InterestRateModel>(world, from, [
          baseRate.encode(),
          multiplier.encode(),
          jump.encode(),
          kink.val,
        ...
        description: `JumpRateModel baseRate=${baseRate
          ...
          .encode()
          .toString()} kink=${kink.encode().toString()} roof=${roof
          .encode()
          .toString()}`,
