...
import { getJoeValue, joeFetchers } from "./Value/JoeValue";
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case "marketStruct":
          ...
          let isJoeKey = "0x" + toBN(newKey).add(toBN(3)).toString(16);
          let isJoeStr = await world.web3.eth.getStorageAt(addr.val, isJoeKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isJoeStr, 1)),
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Joe ...joeArgs" - Returns Joe value
    `,
    "Joe",
    [new Arg("res", getJoeValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: joeFetchers() }
  ),
];

let contractFetchers = [
  { contract: "Counter", implicit: false },
  { contract: "JoeLens", implicit: false },
];
