...
async function genInterestRateModel(
  world: World,
  from: string,
  params: Event
): Promise<World> {
  ...
  return [
    ...
    new View<{ interestRateModelArg: StringV; apiKey: StringV }>(
      ...
      async (world, { interestRateModelArg, apiKey }) => {
        let [interestRateModel, name, data] = await getInterestRateModelData(
          world,
          interestRateModelArg.val
        );

        return await verifyInterestRateModel(
          world,
          interestRateModel,
          apiKey.val,
          name,
...
export async function processInterestRateModelEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
