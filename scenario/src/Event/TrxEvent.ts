...
async function setTrxValue(world: World, value: encodedNumber): Promise<World> {
  return world.update("trxInvokationOpts", (t) =>
    t.set("value", value.toString())
  );
}

async function setTrxGasPrice(
  world: World,
  gasPrice: encodedNumber
): Promise<World> {
  return world.update("trxInvokationOpts", (t) =>
    t.set("gasPrice", gasPrice.toString())
  );
}
  ...
  return [
    new Command<{ amount: NumberV; event: EventV }>(
      ...
      async (world, from, { amount, event }) =>
        processCoreEvent(
          await setTrxValue(world, amount.encode()),
          event.val,
          from
        )
    ),
    new Command<{ gasPrice: NumberV; event: EventV }>(
      ...
      async (world, from, { gasPrice, event }) =>
        processCoreEvent(
          await setTrxGasPrice(world, gasPrice.encode()),
          event.val,
          from
        )
    ),
